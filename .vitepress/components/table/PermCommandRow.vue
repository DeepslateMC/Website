...
const showCopiedState = (text) => {
  ...
  setTimeout(() => {
    if (copiedItem.value === text) copiedItem.value = null
  }, 2000)
}
  ...
  <div class="perm-row" :class="{ 'is-grouped': grouped }">
    <div class="col-cmd">
      ...
      <div class="badge-container">
        <span v-for="cmd in toArray(commands)" :key="cmd" class="badge cmd-badge clickable" :class="{ 'is-copied': copiedItem === cmd }" @click="copyToClipboard(cmd)" title="Klicken zum Kopieren">{{ cmd }}</span>
        <span v-for="alias in toArray(aliases)" :key="alias" class="badge alias-badge clickable" :class="{ 'is-copied': copiedItem === alias }" @click="copyToClipboard(alias)" title="Klicken zum Kopieren">{{ alias }}</span>
      </div>
    </div>

    <div class="col-perm">
      <span class="mobile-label">Permission:</span>
      <span class="badge perm-badge clickable" :class="{ 'is-copied': copiedItem === permission }" @click="copyToClipboard(permission)" title="Klicken zum Kopieren">{{ permission }}</span>
    </div>
  </div>
</template>
...
.badge {
  position: relative; padding: 4px 10px; border-radius: 6px; font-family: var(--vp-font-family-mono); font-size: 0.9em; font-weight: 600; white-space: nowrap; transition: all 0.2s ease;
}
...
.badge.is-copied::after { content: 'Kopiert!'; position: absolute; bottom: 120%; left: 50%; transform: translateX(-50%); background-color: var(--vp-c-text-1); color: var(--vp-c-bg); padding: 4px 8px; border-radius: 4px; font-family: var(--vp-font-family-base); font-size: 0.75rem; font-weight: bold; pointer-events: none; animation: popIn 0.2s forwards; }
...
@media (max-width: 768px) {
  ...
  .perm-row.is-grouped { padding-bottom: 20px; }
